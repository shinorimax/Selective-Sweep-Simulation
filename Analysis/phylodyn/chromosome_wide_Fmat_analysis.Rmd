...
F_weighted_neutrals <- readRDS("/Users/yagishinnosuke/Documents/2025-2026 Stanford/Palacios Lab/bim-paper/Simulations/data/F_weighted_average_neutrals.rds")
F_weighted_selectives_s0.01_f0.75 <- readRDS("/Users/yagishinnosuke/Documents/2025-2026 Stanford/Palacios Lab/bim-paper/Simulations/data/F_weighted_average_selectives_s0.01_f0.75.rds")

# Follow "s={s}F={F}" naming rule
scenario_tag <- "s=0.01F=0.75"
...
F_weighted_list_neutral_test <- F_weighted_neutrals[1: half_idx -1]
...
F_mean_neutral <- readRDS("/Users/yagishinnosuke/Documents/2025-2026 Stanford/Palacios Lab/bim-paper/Simulations/data/F_empirical_kingman.rds")
...
### Export results to graph in python
...
out_csv <- file.path(roc_dir, sprintf("D1F_scores_%s.csv", scenario_tag))
...
# roc_dir <- file.path("..", "..", "SliM", "roc")   # ../../SliM/roc
roc_dir <- "/Users/yagishinnosuke/Documents/2025-2026 Stanford/Palacios Lab/bim-paper/Simulations/roc" 
dir.create(roc_dir, showWarnings = FALSE, recursive = TRUE)
...
### Export results to graph in python for all scenarios
...
for (s in s_vals) for (F in f_vals) {
  ...
  d_selective <- vapply(
    F_weighted_selective,
    function(Fmat) dist_d2(Fmat, F_mean_neutral),
    numeric(1)
  )
  ...
  out_csv <- file.path(roc_dir, sprintf("D2F_scores_%s.csv", scenario_tag))
  write.csv(scores_df, out_csv, row.names = FALSE)
  message("Wrote: ", out_csv, "  (n0=", length(d_neutral_test),
          ", n1=", length(d_selective), ")")
}
...
ggplot(df_dist, aes(x = distance, fill = scenario)) +
  ...
  theme_minimal(base_size = 14) +
  labs(
    title = "Distribution of d0 Distances to Neutral Mean F-matrix",
    x = expression(paste("Distance (", d[1], ")")),
    y = "Density"
  ) +
  scale_fill_manual(values = c("#1b9e77", "#d95f02"))
...
for (s in s_vals) for (F in f_vals) {
  ...
  out_csv <- file.path(roc_dir, sprintf("D0F_scores_%s.csv", scenario_tag))
  write.csv(scores_df, out_csv, row.names = FALSE)
  message("Wrote: ", out_csv, "  (n0=", length(d_neutral_test),
          ", n1=", length(d_selective), ")")
}
...
## Calculate and store d-1 distance around positive mutation to empirical Kingman

```{r}
F_weighted_neutrals <- readRDS("data/F_weighted_average_around_positive_mutation_neutrals.rds")
F_weighted_selectives_s0.01_f0.75 <- readRDS("data/F_weighted_average_around_positive_mutation_selectives_s0.01_f0.75.rds")
...
# --- 3. Load or reference the empirical mean F-matrix ------------------------
F_mean_neutral <- readRDS("data/F_empirical_kingman_around_positive_mutation.rds")
...
for (s in s_vals) for (F in f_vals) {
  ...
  sel_rds <- file.path(
    "data",
    sprintf("F_weighted_average_around_positive_mutation_selectives_s%s_f%s.rds", s_str, f_str)
  )
  ...
  out_csv <- file.path(roc_dir, sprintf("D1FS_scores_%s.csv", scenario_tag))
  write.csv(scores_df, out_csv, row.names = FALSE)
  message("Wrote: ", out_csv, "  (n0=", length(d_neutral_test),
          ", n1=", length(d_selective), ")")
}
