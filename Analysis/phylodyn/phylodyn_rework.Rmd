...
Defining a function to compute pairwise tree distances
```{r}
...
Defining a function to calculate confusion matrix
```{r}
generate_confusion_matrices <- function(scenarios, dist_method = "l1", weighted = FALSE, trees_per_simulation = 4) {
  ...
  for (scenario_idx in seq_along(scenarios)) {
    ...
    # Classify each tree based on centroid distance
    for (sim_idx in seq_along(scenario_list)) {
      for (tree_idx in 1:trees_per_simulation) {
        tree <- scenario_list[[sim_idx]][[tree_idx]]
        true_label <- ifelse(tree_idx %in% c(1,2), 1, 2)  # Merge into two classes

        # Compute distance to each merged centroid
        distances_to_centroids <- sapply(1:2, function(c_idx) {
          dist_pairwise(tree, centroids[[c_idx]], dist.method = dist_method, weighted = weighted)
        })
        ...
        if (current_count %% progress_step == 0) {
          cat(sprintf("Scenario %d Progress: %.1f%% completed\n", scenario_idx, (current_count / total_trees) * 100))
          flush.console()  # Ensures immediate printing
        }
      }
    }

    # Store the confusion matrix for this scenario
...
MDS approximation of confusion matrix
```{r}
...
first_tree <- scenarios[[3]][[2]][[1]]
second_tree <- scenarios[[3]][[2]][[2]]
third_tree <- scenarios[[3]][[2]][[3]]
fourth_tree <- scenarios[[3]][[2]][[4]]
...
```{r}
# cm_l1uw <- generate_confusion_matrices(scenarios, dist_method = "l1", weighted = FALSE, trees_per_simulation = 4)
```

```{r}
l1uw_cm <- generate_mds_confusion_matrices(distance_matrices_l1_uw)
l1uw_cm
```
...
Generate confusion matrix for l1w
```{r}
l1w_cm <- generate_mds_confusion_matrices(distance_matrices_l1_w)
l1w_cm
```
...
Generate confusion matrix for l2uw
```{r}
l2uw_cm <- generate_mds_confusion_matrices(distance_matrices_l2_uw)
l2uw_cm
```
...
Generate confusion matrix for l2w
```{r}
l2w_cm <- generate_mds_confusion_matrices(distance_matrices_l2_w)
l2w_cm
```

Plot distribution of pairwise distances (l1uw)
```{r}
# Loop through scenarios and simulations to compute pairwise distances
plot_data <- data.frame()

# Loop through scenarios and simulations to compute pairwise distances
par(mfrow = c(2, 2))  # Arrange plots in a 2x2 grid
for (i in seq_along(scenarios)) {
  scenario_number <- as.character(i)
  scenario_data <- scenarios[[i]][1:50]
  
  for (tree_type in 1:4) {  # Loop over tree types
    tree_distances <- lapply(scenario_data, function(sim) sim[[tree_type]])
    # dmat_list <- lapply(tree_distances, dist_matrix)
    dmat_list <- dist_matrix(tree_distances, "l1", FALSE)
    
    # Extract upper triangular part of distance matrices
    list_d <- dmat_list[upper.tri(dmat_list)]
    
    # Store data for ggplot
    plot_data <- rbind(plot_data, data.frame(Scenario = paste("Scenario ", scenario_number), TreeType = as.factor(tree_type), Distance = list_d))
  }
}
...
ggplot(plot_data, aes(x = Distance, fill = TreeType)) +
  ...
  facet_wrap(~Scenario) +
  theme_minimal() +
  labs(title = "Pairwise Tree Distance Distributions (l1, unweighted)", x = "Pairwise Distance", y = "Density", fill = "Tree Type")
```
Plot distribution of pairwise distances (l1w)
```{r}
# Loop through scenarios and simulations to compute pairwise distances
plot_data <- data.frame()
...
for (i in seq_along(scenarios)) {
  ...
  for (tree_type in 1:4) {  # Loop over tree types
    tree_distances <- lapply(scenario_data, function(sim) sim[[tree_type]])
    # dmat_list <- lapply(tree_distances, dist_matrix)
    dmat_list <- dist_matrix(tree_distances, "l1", TRUE)
...
ggplot(plot_data, aes(x = Distance, fill = TreeType)) +
  ...
  facet_wrap(~Scenario) +
  theme_minimal() +
  labs(title = "Pairwise Tree Distance Distributions (l1, weighted)", x = "Pairwise Distance", y = "Density", fill = "Tree Type")
```

Plot distribution of pairwise distances (l2uw)
```{r}
# Loop through scenarios and simulations to compute pairwise distances
plot_data <- data.frame()
...
for (i in seq_along(scenarios)) {
  ...
  for (tree_type in 1:4) {  # Loop over tree types
    tree_distances <- lapply(scenario_data, function(sim) sim[[tree_type]])
    # dmat_list <- lapply(tree_distances, dist_matrix)
    dmat_list <- dist_matrix(tree_distances, "l2", FALSE)
...
ggplot(plot_data, aes(x = Distance, fill = TreeType)) +
  ...
  facet_wrap(~Scenario) +
  theme_minimal() +
  labs(title = "Pairwise Tree Distance Distributions (l2, unweighted)", x = "Pairwise Distance", y = "Density", fill = "Tree Type")
```

Plot distribution of pairwise distances (l2w)
```{r}
# Loop through scenarios and simulations to compute pairwise distances
plot_data <- data.frame()
...
for (i in seq_along(scenarios)) {
  ...
  for (tree_type in 1:4) {  # Loop over tree types
    tree_distances <- lapply(scenario_data, function(sim) sim[[tree_type]])
    # dmat_list <- lapply(tree_distances, dist_matrix)
    dmat_list <- dist_matrix(tree_distances, "l2", TRUE)
...
ggplot(plot_data, aes(x = Distance, fill = TreeType)) +
  ...
  facet_wrap(~Scenario) +
  theme_minimal() +
  labs(title = "Pairwise Tree Distance Distributions (l2, weighted)", x = "Pairwise Distance", y = "Density", fill = "Tree Type")
```
...
<!-- ```{r} -->
<!-- extract_features <- function(distance_matrices) { -->
<!--   feature_list <- list() -->
<!--   labels <- c() -->

<!--   for (scenario_idx in seq_along(distance_matrices)) { -->
<!--     for (sim_idx in seq_along(distance_matrices[[scenario_idx]])) { -->
<!--       dmat <- distance_matrices[[scenario_idx]][[sim_idx]] -->

<!--       # Extract upper triangle (excluding diagonal) -->
<!--       feature_vector <- as.vector(dmat[upper.tri(dmat)]) -->

<!--       # Store extracted features -->
<!--       feature_list <- append(feature_list, list(feature_vector)) -->

<!--       # Generate labels (Tree types: 1,2,3,4) -->
<!--       labels <- c(labels, rep(1:4, each=1))  # Assuming tree types are fixed per 4x4 matrix -->
<!--     } -->
<!--   } -->

<!--   # Convert to data frame -->
<!--   feature_matrix <- do.call(rbind, feature_list) -->
<!--   return(list(features = feature_matrix, labels = labels)) -->
<!-- } -->
<!-- ``` -->
...
<!-- data <- extract_features(distance_matrices_l1_uw) -->
