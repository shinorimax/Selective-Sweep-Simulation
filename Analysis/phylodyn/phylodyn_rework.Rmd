...
```{r}
# Coefficient values for scenarios 1–4
select_coef <- c("0.001", "0.01", "0.1", "0")

# Define the plotting order: 4 → 1 → 2 → 3
plot_order <- c(4, 1, 2, 3)

plot_data <- data.frame()

for (i in plot_order) {
  scenario_data <- scenarios[[i]][1:50]
  coef_label <- select_coef[i]  # This correctly maps i to its coefficient
  cat("Processing Scenario", i, " (Selection Coefficient", coef_label, ")\n")
  
  for (tree_type in 1:4) {
    ...
    plot_data <- rbind(
      plot_data,
      data.frame(
        Scenario = paste("Selection Coefficient", coef_label),
        TreeType = as.factor(tree_type),
        Distance = list_d
      )
    )
  }
...
# Explicitly set the factor level order for correct facet display
plot_data$Scenario <- factor(
  plot_data$Scenario,
  levels = paste("Selection Coefficient", select_coef[plot_order])
)
...
ggplot(plot_data, aes(x = Distance, fill = TreeType)) +
  ...
  theme_minimal() +
  labs(
    title = "Pairwise Tree Distance Distributions (l1, unweighted)",
    x = "Pairwise Distance", y = "Density", fill = "Tree Type"
  )
```
...
ggplot(plot_data, aes(x = Distance, fill = TreeType)) +
  ...
  theme_minimal() +
  labs(
    title = "Pairwise Tree Distance Distributions (l1, weighted)",
    x = "Pairwise Distance", y = "Density", fill = "Tree Type"
  )
```
...
ggplot(plot_data, aes(x = Distance, fill = TreeType)) +
  ...
  theme_minimal() +
  labs(
    title = "Pairwise Tree Distance Distributions (l2, unweighted)",
    x = "Pairwise Distance", y = "Density", fill = "Tree Type"
  )
```
...
ggplot(plot_data, aes(x = Distance, fill = TreeType)) +
  ...
  theme_minimal() +
  labs(
    title = "Pairwise Tree Distance Distributions (l2, weighted)",
    x = "Pairwise Distance", y = "Density", fill = "Tree Type"
  )
```
