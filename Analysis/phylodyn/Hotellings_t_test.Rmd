...
hotelling_score <- function(X, Y) {
  # X: neutral F-vectors for this replicate
  # Y: selective F-vectors for same replicate

  out <- ICSNP::HotellingsT2(X, Y)
  # return either T² or -log10(p)
  return(-log10(out$p.value))
}

scores <- c()
labels <- c()   # "Neutral" or "Selective"

for (r in seq_along(rep_list)) {

    X_neu_r <- do.call(rbind, lapply(neutral_list[[r]], vecF))
    Y_sel_r <- do.call(rbind, lapply(selective_list[[r]], vecF))

    score_r <- hotelling_score(X_neu_r, Y_sel_r)

    # label replicates under neutral simulation as "Neutral"
    labels <- c(labels, "Neutral")
    scores <- c(scores, score_r)
}

labels <- c(rep("Neutral", n_neu_rep),
            rep("Selective", n_sel_rep))
