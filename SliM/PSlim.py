...
class Experiment:
    ...
    #     """
    #     Run the external 'BIM' python script to compute per-tree statistics.
    #     BIM: path to python script (expects: BIM <trees_csv> N --stats=all --eta=... --out=...)
    #     setid: key in self.Args
    #     now: shard count (parallel chunks)
    #     arg: extra CLI args appended to BIM call
    #     """
    #     Arg = self.Args[setid]
    #     eta = Arg['etapath']
    #     simIDs = self.simIDs[setid]
    #     simIDs = [f"trees/r{i}.trees" for i in simIDs] + [f"trees/ir{i}.trees" for i in simIDs]
    #     n = len(simIDs)
    #     sim_iter = iter(simIDs)

    #     # chunk sizes
    #     jc = [n // now] * now
    #     for j in range(n - sum(jc)):
    #         jc[j] += 1

    #     msgs = []
    #     self.outs[setid] = []
    #     for j in jc:
    #         out = os.path.join('outs', f"{np.random.randint(1e8, 1e9-1):09d}.csv")
    #         self.outs[setid].append(out)
    #         trees = ','.join([next(sim_iter) for _ in range(j)])
    #         job = ' '.join([
    #             sys.executable, BIM, trees, str(Arg['N']),
    #             '--stats=all', f"--eta={eta}", f"--out={out}"
    #         ])
    #         job = job + ' ' + arg

    #         if srun is None:
    #             msg = subprocess.run(job, shell=True, capture_output=True, text=True)
    #             if msg.returncode != 0:
    #                 raise RuntimeError(f"[BIM failed]\nCMD: {job}\nSTDOUT:\n{msg.stdout}\nSTDERR:\n{msg.stderr}")
    #             msgs.append(msg.stdout)
    #         else:
    #             msgs.append(srun.run(job))
    ...
    # def est(self, BIM, setid, now=2, srun=None, arg=""):
    #     """
    #     BIM: either 'bim' (installed console script) or a path to BIM.py
    #     """
    #     Arg = self.Args[setid]
    #     eta = Arg['etapath']
    #     simIDs = self.simIDs[setid]
    ...
    #     # parse extra args once (as a list)
    #     extra_args = arg.split() if isinstance(arg, str) else list(arg)

    #     msgs = []
    #     self.outs[setid] = []

    #     # detect how to invoke BIM
    #     use_cli = (os.path.sep not in BIM) and (shutil.which(BIM) is not None)
    #     py_prefix = [] if use_cli else [sys.executable]
    ...
    #         # argv = py_prefix + [BIM, trees, str(Arg['N']),
    #         #                     "--stat=all", f"--eta={eta}", f"--out={out}"] + extra_args

    #         # --- normalize legacy flags in `arg` ---
    #         if isinstance(arg, str):
    #             # map old flags to new CLI names
    #             arg = arg.replace("--stats=", "--stat=")
    #             arg = arg.replace("--treew=", "--weights=")
    #             extra_args = arg.split()
    #         else:
    #             # if already a list, map in-place
    #             extra_args = []
    #             for tok in arg:
    #                 tok = tok.replace("--stats=", "--stat=")
    #                 tok = tok.replace("--treew=", "--weights=")
    #                 extra_args.append(tok)

    #         # --- brace-group the tree paths (CLI supports {a,b,c}) ---
    #         tree_group = "{" + ",".join([next(sim_iter) for _ in range(j)]) + "}"

    #         # --- build argv (no shell) ---
    #         # If BIM is a console script like '/opt/.../bin/bim', call it directly.
    #         # If BIM points to a '.../BIM.py', prefix with sys.executable instead.
    #         use_cli = (os.path.sep not in BIM) or os.access(BIM, os.X_OK)
    #         argv = ([BIM] if use_cli else [sys.executable, BIM]) + [
    #             tree_group,
    #             str(Arg['N']),
    #             "--stat=all",
    #             f"--eta={eta}",
    ...
    #         else:
    #             # if you later use HPC, pass the joined command to srun
    ...
    # def est(self, BIM, setid, now=2, srun=None, arg=""):
    #     import shutil, sys, os, numpy as np, subprocess

    #     Arg = self.Args[setid]
    #     eta = Arg['etapath']

    #     # Build the full list of r/ir trees
    #     simIDs = self.simIDs[setid]
    #     all_trees = [f"trees/r{i}.trees" for i in simIDs] + [f"trees/ir{i}.trees" for i in simIDs]
    #     n = len(all_trees)
    #     sim_iter = iter(all_trees)

    #     # Evenly split into `now` chunks
    ...
    #         arg = arg.replace("--treew=", "--weights=")  # old -> new
    #         extra_args = arg.split()
    #     else:
    #         extra_args = []
    #         for tok in arg:
    ...
    #     # Determine how to invoke BIM
    #     use_cli = (os.path.sep in BIM and os.access(BIM, os.X_OK)) or shutil.which(BIM)
    #     prefix = [BIM] if use_cli else [sys.executable, BIM]

    #     msgs = []
    #     self.outs[setid] = []
    #     for jsz in chunk_sizes:
    #         if jsz == 0:
    #             continue
    #         out = os.path.join('outs', f"{np.random.randint(1e8, 1_000_000_000-1):09d}.csv")
    #         self.outs[setid].append(out)

    #         # Pass each tree path as a separate positional arg
    #         trees_chunk = [next(sim_iter) for _ in range(jsz)]

    #         argv = prefix + trees_chunk + [
    #             str(Arg['N']),
    #             "--stat=all",
    #             f"--eta={eta}",
    #             f"--out={out}",
    ...
    def est(self, BIM, setid, now=2, srun=None, arg=""):
        ...
        simIDs = self.simIDs[setid]
        want = [f"trees/r{i}.trees" for i in simIDs] + [f"trees/ir{i}.trees" for i in simIDs]
